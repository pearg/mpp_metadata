...
```{r}
# Filenames
capture_filename <- "/Users/jess/Work/2017_pearg/mpp_metadata/data/Btr2016_Master_25July2017_FINAL.xlsx"
genotype_filename <- "/Users/jess/Work/2017_pearg/mpp_metadata/data/MPP_Master_Genotype_Phenotype_March2017\ copy.xlsx"
```
...
apply(data[,c("body", "head", "tail", "weight")], 2, function(x) {
  quantile(x, probs=c(seq(0,0.75,by=0.25), 0.8, 0.9, 0.95, 0.98, 0.99, 0.995, 1), 
           na.rm=TRUE)})
...
# "ldop pos not preg, pos bred in prev years"
index <- data$reproductive_condition == "ldop pos not preg, pos bred in prev years"
data[index,"reproductive_condition"] <- "ldop"
data[index,"pregnancy_status"] <- "likely not pregnant"
data[index,"misc_reproductive_notes"] <- 
...
x <- str_detect(data$reproductive_condition, 
                "^(juv p|Juv P LP unused clean n~0.5( much FIP)?)$")
data[x,"life_stage"] <- "immature adult"

# Keep FIP info if available
# data[x,"reproductive_condition"] <- data[x,"reproductive_condition"] %>% 
#   str_replace("^(juv p|Juv P LP unused clean n~0.5)( )?", "")

data[x,"other"] <- append_note(data[x,"reproductive_condition"] %>%
  str_replace("^(juv p|Juv P LP unused clean n~0.5)( )?", ""),
  data[x,"other"])

# Remove juvenile pouch values since they're uninformative
data[x,"reproductive_condition"] <- ""
```
...
# Append comment to other column
data[x,"other"] <- append_note(new_value, data[x,"other"])
...
# "; lmop "
index <- data$reproductive_condition == "; lmop "
stopifnot(data[index,"pouch_condition"] == "lmop")
data[index,"reproductive_condition"] <- ""
```
...
data[x, "lactation_status"] <- "almost post lactating"
...
index <- data$reproductive_condition == ", half sw nip, half FIP"
data[index,"nipple_condition"] <- "half swn"
data[index,"other"] <- append_note("half FIP", data[index,"other"])
data[index,"reproductive_condition"] <- ""

index <- data$reproductive_condition == ", no FIP, 2 half sw nip"
data[index,"nipple_condition"] <- "2 half swn"
data[index,"other"] <- append_note("no FIP", data[index,"other"])
data[index,"reproductive_condition"] <- ""
...
```{r}
x <- str_detect(data$reproductive_condition, "FIP")

# What values?
data[x,"reproductive_condition"] %>% unique

# "lpd  FIP" - going to assume this means "ldp"
index <- data$reproductive_condition == "lpd  FIP"
data[index, "pouch_condition"] <- "ldp"
data[index, "reproductive_condition"] <- "FIP"
...
# Move to FIP info to other column
data[x,"other"] <- 
  append_note(new_values, data[x,"other"])

# Remove FIP status from rep condition
data[x,"reproductive_condition"] <- ""
```
...
data[index, "nipple_condition"] <- "swn, no milk"   ### TODO: check
data[index, "reproductive_condition"] <- ""
...
# "lpd"
index <- data$reproductive_condition == "lpd"   ### TODO: check
stopifnot(data[index, "pouch_condition"] == "")
data[index, "pouch_condition"] <- "ldp"
data[index, "reproductive_condition"] <- ""

# "no Fat In Pouch"
index <- data$reproductive_condition == "no Fat In Pouch"
data[index, "misc_reproductive_notes"] <- 
  append_note("no fat in pouch", data[index, "misc_reproductive_notes"])
data[index, "reproductive_condition"] <- ""
...
tidy_data <- tidy_data %>% select(
  ...
  pregnancy_status, pouch_condition, number_of_pouch_young, 
  pouch_young_size_in_mm, lactation_status, nipple_condition, 
  testis_size_status, testis_size_in_mm, misc_reproductive_notes,
  other_notes,
